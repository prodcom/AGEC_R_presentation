...
Note that the 'Preview' function does **not** execute all the code, unlike the `knit` functions. It provides a preview based on the R session that is in progress. 

You will need to execute all the code beforehand:

- Ctrl+Alt+R: Run all chunks
- Ctrl+Alt+P: Run all chunks above cursor
...
```{r AccessData, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
# Libraries
library(tidyverse)
library(readxl)
library(pander) 
...
We can see that there are `r length(apra)` variables for `r nrow(apra)` superannuation funds. 

It is also evident that there are a number of numeric variables -- such as total assets -- that have been imported as character variables. This is because some of the data relating to small funds has been withheld and replaced by an `*`.
...
```{r SummariseData, echo=TRUE, message=FALSE, warning=FALSE}
